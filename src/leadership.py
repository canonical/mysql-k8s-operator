...
class _ObjectABCMeta(
    type(ops.framework.Object), type(collections.abc.MutableMapping)
):
...
class _PeerData(
    ...
    def __getitem__(self, key: str) -> Any:
        if not isinstance(key, str):
            raise TypeError(
                f"key must be a string, got {repr(key)} {type(key)}"
            )
...
class _RawCodec(object):
    def encode(self, value: Any) -> str:
        if not isinstance(value, str):
            raise TypeError(
                f"{self.__class__.__name__} only supports str values, got {type(value)}"
            )
        return value
...
class LeadershipSettings(collections.abc.MutableMapping):
    ...
    def _cache(self) -> Dict[str, str]:
        ...
        if cls.__cls_cache is None:
            ...
            cls.__cls_cache = (
                yaml.safe_load(subprocess.check_output(cmd).decode("UTF-8"))
                or {}
            )
    ...
    def __setitem__(self, key: str, value: str):
        if "=" in key:
            ...
            raise RuntimeError(
                f"LeadershipSettings keys may not contain '=', got {key}"
            )
        if value is None:
            value = ""
