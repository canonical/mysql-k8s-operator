...
class MySQL(MySQLBase):
    ...
    def _build_cluster_tcp_executor(self, host: str, port: int = 3306) -> BaseExecutor:
        ...
        client = super()._build_cluster_tcp_executor(host=host, port=port)
        client.set_container(self.container)
        return client

    def _build_instance_tcp_executor(self, host: str, port: int = ADMIN_PORT) -> BaseExecutor:
        """Build an executor for the cluster primary."""
        client = super()._build_instance_tcp_executor(host=host, port=port)
        client.set_container(self.container)
        return client

    def _build_instance_sock_executor(self, socket: str) -> BaseExecutor:
        """Build an executor for the cluster primary."""
        client = super()._build_instance_sock_executor(socket=socket)
        client.set_container(self.container)
        return client
