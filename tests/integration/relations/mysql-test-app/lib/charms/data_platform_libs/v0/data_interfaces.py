...
class CachedSecret:
    ...
    def get_info(self) -> Optional[SecretInfo]:
        """Wrapper function to apply the corresponding call on the Secret object within CachedSecret if any."""  # noqa
        if self.meta:
...
class DataProvides(DataRelation):
    ...
    def _fetch_specific_relation_data(self, relation, fields: Optional[List[str]]) -> dict:
        ...
        NOTE: Since all secret fields are in the Requires side of the databag, we don't need to worry about that  # noqa
        """
        if not relation.app:
            return {}
...
class DataRequires(DataRelation):
    ...
    def _fetch_specific_relation_data(
            ...
            if fields:
                ...
                for group in secret_fieldnames_grouped:
                    if contents := self._retrieve_group_secret_contents(
                        ...
                        # If it wasn't found as a secret, let's give it a 2nd chance as "normal" field  # noqa
        ...
        # Processing "normal" fields. May include leftover from what we couldn't retrieve as a secret.  # noqa
