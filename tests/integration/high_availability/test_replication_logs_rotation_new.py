...
def start_log_rotate_dispatcher(juju: Juju, unit_name: str) -> None:
    ...
    juju_exec_output = juju.ssh(command="which juju-exec", target=unit_name).strip()
    juju_run_output = juju.ssh(command="which juju-run", target=unit_name).strip()

    dispatch_command = juju_exec_output or juju_run_output
