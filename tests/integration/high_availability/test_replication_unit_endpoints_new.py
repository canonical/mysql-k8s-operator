...
def test_deploy_highly_available_cluster_1(juju: Juju, charm: str) -> None:
    ...
    juju.wait(
        ready=wait_for_apps_status(
            jubilant_backports.all_active, MYSQL_APP_NAME_1, MYSQL_TEST_APP_NAME_1
        ),
...
def test_deploy_highly_available_cluster_2(juju: Juju, charm: str) -> None:
    ...
    juju.wait(
        ready=wait_for_apps_status(
            jubilant_backports.all_active, MYSQL_APP_NAME_2, MYSQL_TEST_APP_NAME_2
        ),
        error=jubilant_backports.any_blocked,
        timeout=20 * MINUTE_SECS,
    )
...
def test_labeling_of_k8s_endpoints(juju: Juju) -> None:
    ...
    juju.wait(
        ready=wait_for_database_related(MYSQL_APP_NAME_1, MYSQL_TEST_APP_NAME_1),
        timeout=20 * MINUTE_SECS,
    )
    check_endpoint_addresses(juju, MYSQL_APP_NAME_1)

    juju.wait(
        ready=wait_for_database_related(MYSQL_APP_NAME_2, MYSQL_TEST_APP_NAME_2),
        timeout=20 * MINUTE_SECS,
    )
...
def wait_for_database_related(app_1_name: str, app_2_name: str):
    """Returns whether the provided apps are related via the 'database' endpoint."""
    return lambda status: all((
        status.apps[app_1_name].relations["database"][0].related_app == app_2_name,
        status.apps[app_2_name].relations["database"][0].related_app == app_1_name,
    ))
