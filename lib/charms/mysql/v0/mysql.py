...
class MySQLBase(ABC):
    ...
    def get_cluster_status(self, extended: bool | None = False) -> dict | None:
        """Get the cluster status dictionary."""
        try:
            # NOTE: This operation used to be done by the server-config user
            status = self._cluster_client_tcp.fetch_cluster_status(self.cluster_name, extended)
        except ExecutionError:
            return None
        else:
            return status

    def get_cluster_set_status(self, extended: int | None = 1) -> dict | None:
        """Get the cluster-set status dictionary."""
        try:
            # NOTE: This operation used to be done by the server-config user
            status = self._cluster_client_tcp.fetch_cluster_set_status(bool(extended))
        except ExecutionError:
            return None
        else:
            return status
