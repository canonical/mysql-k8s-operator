...
from mysql_shell.models.roles import (
    ClusterRole,
    InstanceRole,
)
from mysql_shell.models.statement import (
    LogType,
)
from mysql_shell.models.statement import (
    VariableScope as Scope,
)
from mysql_shell.models.status import (
    ClusterGlobalStatus,
    InstanceStatus,
)
...
class MySQLCharmBase(CharmBase, ABC):
    ...
    def only_one_cluster_node_thats_uninitialized(self) -> bool | None:
        ...
        for unit in self.app_units:
            total_online_cluster_nodes += self._mysql.get_cluster_node_count(
                from_instance=self.get_unit_address(unit, PEER),
                node_status=InstanceStatus.ONLINE,
            )
    ...
    def cluster_fully_initialized(self) -> bool:
        ...
        """
        return self._mysql.get_cluster_node_count(node_status=InstanceStatus.ONLINE) == min(
    ...
    def get_cluster_endpoints(self, relation_name: str) -> tuple[str, str, str]:
        ...
        for k, v in repl_topology.items():
            ...
            if v["status"] == InstanceStatus.RECOVERING:
                continue

            address = f"{self.get_unit_address(unit_labels[k], relation_name)}:3306"

            if v["status"] != InstanceStatus.ONLINE:
                no_endpoints.add(address)
            if v["status"] == InstanceStatus.ONLINE and v["mode"] == "r/o":
                ro_endpoints.add(address)
            if v["status"] == InstanceStatus.ONLINE and v["mode"] == "r/w" and not repl_cluster:
...
class MySQLBase(ABC):
    ...
    def __init__(
        ...
        self._instance_client_sock = MySQLInstanceClient(
            self._build_instance_sock_executor(socket_path),
            self._quoter,
        )

    def _build_cluster_tcp_executor(self, host: str, port: int = 3306) -> BaseExecutor:
        """Build an executor for the cluster primary."""
    ...
    def _build_instance_tcp_executor(self, host: str, port: int = ADMIN_PORT) -> BaseExecutor:
        """Build an executor for the cluster primary."""
        return self.executor_class(
            conn_details=ConnectionDetails(
    ...
    def _build_instance_sock_executor(self, socket: str) -> BaseExecutor:
        """Build an executor for the cluster primary."""
        return self.executor_class(
            conn_details=ConnectionDetails(
    ...
    def configure_mysql_router_roles(self) -> None:
        ...
        executor = self._build_instance_sock_executor(self.socket_path)
    ...
    def install_plugins(self, plugins: list[str]) -> None:
        ...
        try:
            ...
            self._instance_client_tcp.set_instance_variable(Scope.GLOBAL, "super_read_only", "ON")
    ...
    def does_mysql_user_exist(self, username: str, hostname: str) -> bool:
        ...
        try:
            users = self._instance_client_tcp.search_instance_users(username)
        except ExecutionError as e:
            raise MySQLCheckUserExistenceError() from e

        return User(username, hostname) in users
    ...
    def configure_instance(self, create_cluster_admin: bool = True) -> None:
        ...
        try:
            ...
            self._cluster_client_tcp.setup_instance_before_cluster(options=options)
        except ExecutionError as e:
    ...
    def cluster_metadata_exists(self, from_instance: str | None = None) -> bool:
        ...
        if from_instance:
            ...
            client = MySQLInstanceClient(
                self._build_instance_sock_executor(self.socket_path),
                self._quoter,
            )
    ...
    def is_instance_configured_for_innodb(self, instance_address: str) -> bool:
        ...
        client = MySQLClusterClient(
            executor=self._build_cluster_tcp_executor(instance_address),
        )

        # NOTE: This operation used to be done by the server-config user
        return client.check_instance_before_cluster()
    ...
    def is_instance_in_cluster(self, unit_label: str) -> bool:
        ...
        try:
            ...
            return unit_status in (InstanceStatus.ONLINE, InstanceStatus.RECOVERING)
    ...
    def get_replica_cluster_status(self, replica_cluster_name: str | None = None) -> str:
        ...
        return cluster_set_status["clusters"][replica_cluster_name]["globalStatus"]

    def get_cluster_node_count(
        self,
        from_instance: str | None = None,
        node_status: InstanceStatus | None = None,
    ) -> int:
        ...
        node_statuses = [node_status] if node_status else list(InstanceStatus)

        try:
            status = self.get_cluster_status(from_instance, extended=True)
        except ExecutionError:
            logger.warning("Failed to get node count")
            return 0

        count = 0
        for _, member in status["defaultReplicaSet"]["topology"].items():
            if member["status"] in node_statuses:
                count += 1
    ...
    def remove_instance(
        ...
        Args:
            ...
            from_instance: (optional) The instance address to acquire the lock on.
    ...
    def dissolve_cluster(self, force: bool = True) -> None:
        ...
        if force:
            self._cluster_client_tcp.destroy_cluster(self.cluster_name)
    ...
    def _acquire_lock(self, executor: BaseExecutor, unit_label: str, unit_task: str) -> bool:
        ...
        query = self._lock_query_builder.build_acquire_query(
            task=unit_task,
            instance=unit_label,
        )
    ...
    def get_cluster_primary_address(
        self,
        connect_instance_host: str | None = None,
        connect_instance_port: str | None = None,
    ) -> str | None:
        """Get the cluster primary's address."""
        if not connect_instance_host:
            connect_instance_host = self.instance_address
        if not connect_instance_port:
            connect_instance_port = 3306

        executor = self._build_cluster_tcp_executor(connect_instance_host, connect_instance_port)
        client = MySQLClusterClient(executor)
    ...
    def get_cluster_set_global_primary_address(self) -> str | None:
        """Get the cluster set global primary's address."""
        try:
            status = self._cluster_client_tcp.fetch_cluster_set_status()
        except ExecutionError as e:
            raise MySQLGetClusterPrimaryAddressError() from e
        else:
            primary_address = status["globalPrimaryInstance"]
            primary_address = primary_address.split(":")[0]
    ...
    def update_user_password(self, username: str, new_password: str, host: str = "%") -> None:
        ...
        instance_address = self.get_cluster_set_global_primary_address()
        if not instance_address:
    ...
    def get_member_role(self) -> InstanceRole:
    ...
    def get_member_state(self) -> InstanceStatus:
    ...
    def is_cluster_replica(self) -> bool | None:
        """Check if this cluster is a replica in a cluster set."""
        cluster_set_status = self.get_cluster_set_status(extended=0)
        if not cluster_set_status:
            return

        cluster_spec = cluster_set_status["clusters"][self.cluster_name]
        cluster_role = cluster_spec["clusterRole"]

        return cluster_role == ClusterRole.REPLICA
    ...
    def hold_if_recovering(self) -> None:
        ...
        while True:
            try:
                ...
                break
            if member_state == InstanceStatus.RECOVERING:
    ...
    def kill_client_sessions(self) -> None:
        ...
        try:
            self._instance_client_tcp.stop_instance_processes(
                self._instance_client_tcp.search_instance_connections("%"),
            )
        except ExecutionError as e:
    ...
    def check_mysqlsh_connection(self) -> bool:
        ...
        sock_executor = self._build_instance_sock_executor(self.socket_path)
        tcp_executor = self._build_instance_tcp_executor(self.instance_address)

        try:
            tcp_executor.check_connection()
            return True
        except ExecutionError:
            logger.warning("Failed to connect to MySQL using a TCP connection")

        try:
            sock_executor.check_connection()
            return True
        except ExecutionError:
            logger.error("Failed to connect to MySQL using a socket connection")
            return False
    ...
    def flush_mysql_logs(self, log_types: LogType | list[LogType]) -> None:
        ...
        try:
            ...
            if any(log_type == "AUDIT" for log_type in log_types):
                executor.execute_sql("audit_log_rotate()")
        except ExecutionError:
            pass
